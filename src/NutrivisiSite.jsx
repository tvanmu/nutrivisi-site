...
const CustomCursor = ({ disabled }) => {
  ...
  return (
    <>
      <div
        className="fixed top-0 left-0 w-10 h-10 border border-[#F5A623]/40 rounded-full pointer-events-none z-[9999] transition-all duration-300 ease-out items-center justify-center mix-blend-screen hidden sm:flex"
        style={{
          transform: `translate(${position.x - 20}px, ${position.y - 20}px) scale(${isHovering ? 1.6 : 1})`,
          backgroundColor: isHovering ? 'rgba(245,166,35,0.08)' : 'transparent',
          boxShadow: isHovering ? '0 0 18px rgba(245,166,35,0.2)' : 'none',
          opacity: visible ? 1 : 0,
        }}
      />
      <div
...
const AdvisoryLayersVisual = () => (
  <svg viewBox="0 0 400 400" className="w-full h-full max-w-[320px] opacity-85">
    ...
    <circle cx="200" cy="200" r="180" fill="none" stroke="#4BB8D4" strokeWidth="0.8" strokeDasharray="2 8" />
    <circle cx="200" cy="200" r="130" fill="none" stroke="#F5A623" strokeWidth="0.6" opacity="0.5" />
    <circle cx="200" cy="200" r="80" fill="none" stroke="#4BB8D4" strokeWidth="1" opacity="0.7" />
    <circle cx="200" cy="200" r="38" fill="url(#adv-core)" />
    <circle cx="200" cy="200" r="12" fill="#F5A623" className="animate-pulse" />
    ...
    {/* Orbit dots */}
    <circle cx="200" cy="70" r="4" fill="#F5A623" />
    <circle cx="330" cy="200" r="4" fill="#4BB8D4" />
    <circle cx="200" cy="330" r="4" fill="#F5A623" />
    <circle cx="70" cy="200" r="4" fill="#4BB8D4" />
    {/* Tier labels */}
    <text x="200" y="30" textAnchor="middle" fill="#F5A623" fontSize="9" fontFamily="monospace" letterSpacing="3">POLICY</text>
    <text x="200" y="82" textAnchor="middle" fill="#4BB8D4" fontSize="8" fontFamily="monospace" letterSpacing="3" opacity="0.8">RISK</text>
...
const WorkbookVisual = () => (
  <svg viewBox="0 0 400 400" className="w-full h-full max-w-[320px] opacity-90">
    ...
    {/* Main card */}
    <rect x="65" y="45" width="270" height="320" rx="8" fill="url(#wb-page)" stroke="#F5A623" strokeWidth="1.5" />
    {/* Header bar */}
    <rect x="65" y="45" width="270" height="34" rx="8" fill="#F5A623" opacity="0.15" />
    <rect x="65" y="75" width="270" height="1" fill="#F5A623" opacity="0.4" />
    <text x="82" y="67" fill="#F5A623" fontSize="9" fontFamily="monospace" letterSpacing="2">HACCP · VISUELE INSTRUCTIE</text>
    {/* Pictogram grid */}
    <g transform="translate(95, 105)">
      {/* Handwash */}
      <rect x="0" y="0" width="60" height="60" rx="6" fill="#011a24" stroke="#4BB8D4" strokeWidth="1" opacity="0.9" />
      <circle cx="30" cy="22" r="8" fill="none" stroke="#4BB8D4" strokeWidth="1.5" />
      <path d="M 20 36 Q 30 42 40 36 L 40 48 L 20 48 Z" fill="none" stroke="#4BB8D4" strokeWidth="1.5" />
      <circle cx="22" cy="44" r="1.5" fill="#4BB8D4" />
      <circle cx="38" cy="46" r="1.5" fill="#4BB8D4" />
      {/* Temp */}
      <rect x="75" y="0" width="60" height="60" rx="6" fill="#011a24" stroke="#F5A623" strokeWidth="1" opacity="0.9" />
      <rect x="101" y="15" width="8" height="28" rx="2" fill="none" stroke="#F5A623" strokeWidth="1.5" />
      <circle cx="105" cy="45" r="5" fill="#F5A623" />
      <line x1="114" y1="22" x2="120" y2="22" stroke="#F5A623" strokeWidth="1" />
      <line x1="114" y1="30" x2="120" y2="30" stroke="#F5A623" strokeWidth="1" />
      <line x1="114" y1="38" x2="120" y2="38" stroke="#F5A623" strokeWidth="1" />
      {/* Separate */}
      <rect x="150" y="0" width="60" height="60" rx="6" fill="#011a24" stroke="#4BB8D4" strokeWidth="1" opacity="0.9" />
      <rect x="162" y="15" width="16" height="12" rx="1" fill="none" stroke="#4BB8D4" strokeWidth="1.5" />
      <rect x="182" y="33" width="16" height="12" rx="1" fill="none" stroke="#4BB8D4" strokeWidth="1.5" />
      <line x1="170" y1="33" x2="180" y2="22" stroke="#F5A623" strokeWidth="1" strokeDasharray="2 2" />
    </g>
    {/* Caption lines */}
    <rect x="95" y="195" width="210" height="4" rx="2" fill="#F5A623" opacity="0.4" />
    ...
    {/* Footer bar */}
    <rect x="95" y="310" width="210" height="32" rx="4" fill="#F5A623" opacity="0.1" stroke="#F5A623" strokeWidth="0.6" />
    <text x="200" y="330" textAnchor="middle" fill="#F5A623" fontSize="8" fontFamily="monospace" letterSpacing="2">FOOD SAFETY CULTURE</text>
    {/* Decorative tags */}
    <circle cx="320" cy="60" r="12" fill="#F5A623" opacity="0.2" />
    <circle cx="320" cy="60" r="5" fill="#F5A623" />
  </svg>
);
    ...
    <svg viewBox="0 0 400 400" className="w-full h-full max-w-[320px] opacity-90">
      <defs>
        <linearGradient id="cert-line" x1="0" y1="0" x2="0" y2="1">
          ...
          <stop offset="60%" stopColor="#4BB8D4" opacity="0.4" />
          <stop offset="100%" stopColor="#4BB8D4" opacity="0.2" />
        </linearGradient>
      </defs>
      ...
      {items.map((item, i) => {
        ...
        const color = isDone ? '#F5A623' : isActive ? '#F5A623' : '#4BB8D4';
        return (
          <g key={item.label}>
            {isActive && <circle cx="90" cy={y} r="20" fill="none" stroke="#F5A623" strokeWidth="1" opacity="0.4" className="animate-ping" />}
            <circle cx="90" cy={y} r="12" fill={isDone ? '#F5A623' : '#011a24'} stroke={color} strokeWidth="2" />
            {isDone && <path d={`M ${85} ${y} L ${89} ${y + 4} L ${96} ${y - 3}`} stroke="#012330" strokeWidth="2" fill="none" strokeLinecap="round" strokeLinejoin="round" />}
            {isActive && <circle cx="90" cy={y} r="4" fill="#F5A623" className="animate-pulse" />}
            <rect x="115" y={y - 14} width="220" height="28" rx="4" fill={isActive ? '#F5A623' : '#011a24'} fillOpacity={isActive ? 0.15 : 0.6} stroke={color} strokeWidth="0.8" strokeOpacity={isActive ? 1 : 0.4} />
            <text x="125" y={y + 4} fill={isActive ? '#F5A623' : isDone ? '#ffffff' : '#4BB8D4'} fontSize="10" fontFamily="monospace" letterSpacing="2" opacity={item.status === 'pending' ? 0.6 : 1}>{item.label}</text>
            {isDone && <text x="320" y={y + 4} fill="#F5A623" fontSize="9" fontFamily="monospace" letterSpacing="1" opacity="0.7">OK</text>}
            {isActive && <text x="315" y={y + 4} fill="#F5A623" fontSize="9" fontFamily="monospace" letterSpacing="1">··· 72%</text>}
          </g>
        );
      })}
    </svg>
...
const RiskHeatmapVisual = () => {
  ...
  return (
    <svg viewBox="0 0 400 400" className="w-full h-full max-w-[320px] opacity-90">
      {/* Axis labels */}
      <text x="200" y="20" textAnchor="middle" fill="#F5A623" fontSize="9" fontFamily="monospace" letterSpacing="2">RISK MATRIX · 5×5</text>
        ...
        row.map((val, j) => {
          ...
          return (
            <g key={`${i}-${j}`}>
              ...
              {highlighted && (
                <>
                  <rect x={x - 4} y={y - 4} width="58" height="58" rx="5" fill="none" stroke="#F5A623" strokeWidth="1.5" strokeDasharray="3 3" className="animate-pulse" />
      ...
      <g className="animate-[spin_10s_linear_infinite]" style={{ transformOrigin: '200px 200px' }}>
        <path d="M 200 200 L 200 50" stroke="#F5A623" strokeWidth="1.5" opacity="0.35" />
      </g>
      {/* Pinpoint on critical cell */}
      <circle cx="220" cy="115" r="5" fill="#F5A623" className="animate-pulse" />
      <text x="235" y="119" fill="#F5A623" fontSize="8" fontFamily="monospace" letterSpacing="2">ALLERGEN</text>
    </svg>
  );
};
...
const LabelInspectorVisual = () => (
  <svg viewBox="0 0 400 400" className="w-full h-full max-w-[340px] opacity-95">
    ...
    {/* Label body */}
    <rect x="70" y="70" width="260" height="260" rx="6" fill="url(#label-bg)" stroke="#F5A623" strokeWidth="1.5" />
    {/* Label header */}
    <rect x="70" y="70" width="260" height="38" rx="6" fill="#F5A623" opacity="0.15" />
    <rect x="82" y="82" width="70" height="14" rx="2" fill="#F5A623" opacity="0.9" />
    ...
    {/* Ingredients block */}
    <text x="82" y="160" fill="#4BB8D4" fontSize="7" fontFamily="monospace" letterSpacing="2">INGREDIËNTEN</text>
    ...
    <rect x="140" y="192" width="52" height="10" rx="2" fill="#F5A623" opacity="0.3" stroke="#F5A623" strokeWidth="1" />
    <text x="144" y="200" fill="#F5A623" fontSize="7" fontFamily="monospace" fontWeight="bold">GLUTEN</text>
    ...
    {/* Nutritional block */}
    <text x="82" y="220" fill="#4BB8D4" fontSize="7" fontFamily="monospace" letterSpacing="2">VOEDINGSWAARDE / 100G</text>
    ...
    <rect x="240" y="285" width="70" height="20" rx="2" fill="none" stroke="#F5A623" strokeOpacity="0.6" />
    <text x="248" y="298" fill="#F5A623" fontSize="7" fontFamily="monospace">TGT 30.07.26</text>
    {/* Callout 1 — Allergen */}
    <line x1="192" y1="197" x2="355" y2="155" stroke="#F5A623" strokeWidth="1" strokeDasharray="2 2" />
    <circle cx="192" cy="197" r="3" fill="#F5A623" />
    <rect x="340" y="140" width="58" height="30" rx="3" fill="#011a24" stroke="#F5A623" strokeWidth="0.8" />
    <text x="346" y="152" fill="#F5A623" fontSize="7" fontFamily="monospace">ALLERGEN</text>
    <text x="346" y="163" fill="#ffffff" fontSize="6" fontFamily="monospace" opacity="0.7">EU 1169/2011</text>
    {/* Callout 2 — TGT */}
    <line x1="310" y1="295" x2="365" y2="320" stroke="#4BB8D4" strokeWidth="1" strokeDasharray="2 2" />
    <circle cx="310" cy="295" r="3" fill="#4BB8D4" />
    <rect x="340" y="310" width="58" height="28" rx="3" fill="#011a24" stroke="#4BB8D4" strokeWidth="0.8" />
    <text x="346" y="322" fill="#4BB8D4" fontSize="7" fontFamily="monospace">DATE</text>
    <text x="346" y="333" fill="#ffffff" fontSize="6" fontFamily="monospace" opacity="0.7">FAVV CHECK</text>
    {/* Callout 3 — Batch */}
    <line x1="88" y1="295" x2="22" y2="340" stroke="#4BB8D4" strokeWidth="1" strokeDasharray="2 2" />
    <circle cx="88" cy="295" r="3" fill="#4BB8D4" />
    <rect x="4" y="330" width="58" height="28" rx="3" fill="#011a24" stroke="#4BB8D4" strokeWidth="0.8" />
    <text x="10" y="342" fill="#4BB8D4" fontSize="7" fontFamily="monospace">BATCH</text>
    <text x="10" y="353" fill="#ffffff" fontSize="6" fontFamily="monospace" opacity="0.7">TRACEABLE</text>
    {/* Scan line */}
    <line x1="70" y1="230" x2="330" y2="230" stroke="#F5A623" strokeWidth="1" opacity="0.5" className="animate-[labelScan_5s_ease-in-out_infinite]" />
  </svg>
);
      ...
      <h2 className="text-5xl md:text-7xl font-extrabold tracking-tight text-white mb-6">
        {title.split(' ').map((word, i, arr) =>
          i === arr.length - 1 || i === arr.length - 2
            ? <span key={i} className="text-transparent bg-clip-text bg-gradient-to-r from-[#F5A623] to-[#FFC35C]">{word} </span>
  ...
  return (
    <div className="min-h-screen bg-[#012330] font-sans text-slate-200 overflow-x-hidden">
        ...
        <div className="mx-auto flex max-w-7xl items-center justify-between px-4 sm:px-6 lg:px-8">
          ...
          <div className="hidden items-center space-x-6 lg:flex">
            {currentNavItems.map((item) => (
              <a key={item.href} href={item.href} onClick={(e) => { e.preventDefault(); scrollToSection(item.href.replace('#', '')); }} className="relative text-xs font-bold tracking-wider text-teal-100/80 hover:text-[#F5A623] transition-colors group py-2 uppercase interactive-card">
                {item.label}
            ...
            <div className="flex items-center gap-4 pl-4 border-l border-teal-800/50">
              <button onClick={toggleLang} className="w-10 h-10 rounded-full border border-[#F5A623] text-[#F5A623] font-bold text-sm hover:bg-[#F5A623] hover:text-[#012330] transition-colors interactive-card flex items-center justify-center" aria-label="Toggle language">
                {lang === 'NL' ? 'FR' : 'NL'}
              </button>
          ...
          <div className="flex items-center gap-3 lg:hidden">
            <button onClick={toggleLang} className="w-10 h-10 rounded-full border border-[#F5A623] text-[#F5A623] font-bold text-sm flex items-center justify-center" aria-label="Toggle language">
              {lang === 'NL' ? 'FR' : 'NL'}
            </button>
            <button onClick={() => setMobileOpen(!mobileOpen)} className="inline-flex h-11 w-11 items-center justify-center rounded-full border border-teal-800/50 bg-[#011a24] text-white transition hover:border-[#F5A623]/40 hover:text-[#F5A623]" aria-label="Toggle menu">
              ...
              {currentSectors.map((sector, index) => {
                ...
                return (
                  <FadeInSection key={index} delay={index * 100}>
                    <div className="flex flex-col items-center gap-4 text-teal-200/70 font-medium hover:text-[#F5A623] transition-all duration-500 group interactive-card">
      ...
      <section className="relative py-24 bg-[#011722] border-t border-teal-900/40 overflow-hidden">
        <div className="absolute inset-0 opacity-[0.05] pointer-events-none mix-blend-overlay" style={{ backgroundImage: 'linear-gradient(#F5A623 1px, transparent 1px), linear-gradient(90deg, #F5A623 1px, transparent 1px)', backgroundSize: '60px 60px' }}></div>
        <div className="absolute top-0 left-1/4 w-96 h-96 bg-[#F5A623]/8 rounded-full blur-[120px] pointer-events-none"></div>
        <div className="absolute bottom-0 right-1/4 w-96 h-96 bg-[#4BB8D4]/8 rounded-full blur-[120px] pointer-events-none"></div>

        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
          <FadeInSection>
            <div className="flex items-center gap-4 justify-center mb-12">
              <div className="h-px w-16 bg-gradient-to-r from-transparent to-[#F5A623]/60"></div>
              <p className="text-xs font-bold text-[#F5A623] uppercase tracking-[0.35em]">{t.metrics.eyebrow}</p>
              <div className="h-px w-16 bg-gradient-to-l from-transparent to-[#F5A623]/60"></div>
            </div>
          </FadeInSection>
              ...
              <FadeInSection key={index} delay={index * 100}>
                <div className={`relative p-8 md:p-10 text-center group ${index > 0 ? 'md:border-l md:border-teal-800/40' : ''}`}>
                  <div className="absolute top-4 right-4 flex gap-1 opacity-30">
                    <div className="w-1 h-1 rounded-full bg-[#F5A623]"></div>
                    <div className="w-1 h-1 rounded-full bg-[#F5A623]"></div>
                    <div className="w-1 h-1 rounded-full bg-[#F5A623]"></div>
                  </div>
                  <p className="text-4xl md:text-6xl font-extrabold tracking-tighter text-transparent bg-clip-text bg-gradient-to-br from-[#F5A623] via-[#FFC35C] to-[#F5A623] mb-3 transition-all group-hover:scale-105">
                    {metric.value}
                  </p>
                ...
                {currentServices.map((service) => {
                  ...
                  return (
                    <button key={service.id} onClick={() => setActiveServiceId(service.id)}
                      ...
                      {isActive && <div className="absolute left-0 top-0 w-1 h-full bg-[#F5A623] shadow-[0_0_15px_#F5A623]"></div>}
                      <div className="relative flex items-center justify-center w-6 h-6 shrink-0">
                        <div className={`absolute w-2 h-2 rounded-full transition-all duration-500 ${isActive ? 'bg-[#F5A623] scale-100 shadow-[0_0_10px_#F5A623]' : 'bg-teal-700 group-hover:bg-teal-400 scale-75'}`}></div>
            ...
            <div className="lg:col-span-8 relative bg-gradient-to-br from-[#023A4E]/30 to-[#012330]/90 p-10 lg:p-16 flex items-center min-h-[700px] overflow-hidden">
              ...
              <div className="relative z-10 w-full grid grid-cols-1 lg:grid-cols-[1.05fr_1fr] gap-10 items-center" key={activeService.id}>
                ...
                <div className="relative hidden md:flex flex-col items-center justify-center pointer-events-none animate-[blockReveal_1s_ease-out]">
                  <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-72 h-72 bg-[#4BB8D4] rounded-full blur-[120px] opacity-10"></div>
                  <div className="text-xs font-bold uppercase tracking-[0.3em] text-[#F5A623]/70 mb-4 flex items-center gap-2">
                    <span className="h-px w-8 bg-[#F5A623]/40"></span>
      ...
      <section id="waarom" className="py-32 bg-gradient-to-b from-[#012330] via-[#011b26] to-[#012330] relative overflow-hidden border-t border-teal-900/30">
        <div className="absolute top-1/2 left-0 -translate-y-1/2 w-[800px] h-[800px] bg-[#F5A623]/5 rounded-full blur-[180px] pointer-events-none"></div>
        <div className="absolute top-0 right-0 w-[600px] h-[600px] bg-[#4BB8D4]/5 rounded-full blur-[180px] pointer-events-none"></div>
        ...
        <svg className="absolute top-10 right-10 w-40 h-40 opacity-20 pointer-events-none hidden lg:block" viewBox="0 0 200 200">
          <circle cx="100" cy="100" r="90" fill="none" stroke="#F5A623" strokeWidth="0.5" strokeDasharray="2 8" />
          <circle cx="100" cy="100" r="60" fill="none" stroke="#4BB8D4" strokeWidth="0.5" />
          <circle cx="100" cy="100" r="30" fill="none" stroke="#F5A623" strokeWidth="0.5" />
          <circle cx="100" cy="10" r="3" fill="#F5A623" />
        </svg>
              ...
              {t.whySection.items.map((item, index) => {
                ...
                return (
                  <FadeInSection key={item.title} delay={index * 120}>
                    <div className="relative h-full p-7 rounded-[1.75rem] bg-[#023142]/50 backdrop-blur-lg border border-teal-800/40 hover:border-[#F5A623]/50 transition-all duration-500 group overflow-hidden">
                      {/* Corner notation */}
                      <div className="absolute top-4 right-4 font-mono text-[10px] text-[#F5A623]/50 tracking-widest">0{index + 1}</div>
                      <div className="absolute -bottom-8 -left-8 w-32 h-32 bg-[#F5A623]/0 group-hover:bg-[#F5A623]/8 rounded-full blur-2xl transition-all duration-500"></div>

                      <div className="relative">
                        <div className="w-12 h-12 rounded-xl bg-[#011a24] flex items-center justify-center border border-teal-700/40 mb-5 group-hover:border-[#F5A623]/50 group-hover:bg-[#F5A623]/10 transition-all">
      ...
      <section id="kennis" className="py-32 bg-[#012330] relative overflow-hidden border-t border-teal-900/30">
        <div className="absolute top-0 left-1/2 -translate-x-1/2 w-full max-w-5xl h-64 bg-gradient-to-b from-[#4BB8D4]/5 to-transparent blur-[100px] pointer-events-none"></div>
        {/* Dots backdrop */}
        <div className="absolute inset-0 opacity-[0.04] pointer-events-none" style={{ backgroundImage: 'radial-gradient(#F5A623 1px, transparent 1px)', backgroundSize: '30px 30px' }}></div>
              ...
              return (
                <FadeInSection key={item.title} delay={(index % 3) * 100}>
                  <a href="#contact" onClick={(e) => { e.preventDefault(); scrollToSection('contact'); }} className="block h-full group interactive-card">
                    <div className="relative h-full p-8 rounded-2xl bg-[#011a24]/60 backdrop-blur-md border border-teal-800/40 hover:border-[#F5A623]/50 transition-all duration-500 overflow-hidden">
                      {/* Top bar accent */}
                      <div className="absolute top-0 left-0 h-1 w-0 bg-[#F5A623] group-hover:w-full transition-all duration-700"></div>
                      {/* Corner mono index */}
                      <div className="absolute top-4 right-4 font-mono text-[10px] text-teal-500/50 tracking-widest group-hover:text-[#F5A623]/70 transition-colors">
                        K.{String(index + 1).padStart(2, '0')}
                      </div>

                      <div className="flex items-center gap-4 mb-5">
                        <div className="w-11 h-11 rounded-xl bg-[#023142] flex items-center justify-center border border-teal-700/40 group-hover:border-[#F5A623]/60 group-hover:bg-[#F5A623]/10 transition-all">
                          <Icon className="w-5 h-5 text-[#F5A623]" />
                        </div>
                      </div>
                      <h3 className="text-xl font-bold text-white mb-3 leading-tight group-hover:text-[#F5A623] transition-colors">
                        {item.title}
                      </h3>
                      <p className="text-teal-100/75 text-sm leading-relaxed mb-6">{item.text}</p>
                      <div className="inline-flex items-center gap-2 text-xs font-bold uppercase tracking-[0.15em] text-[#F5A623]/80 group-hover:text-[#F5A623]">
              ...
              <div className="space-y-8 pt-8 border-t border-teal-800/40">
                <a href="mailto:info@nutrivisi.be" className="flex items-center gap-6 group selectable">
                  <div className="w-14 h-14 rounded-2xl bg-[#011a24] border border-teal-800 flex items-center justify-center group-hover:border-[#F5A623]/50 group-hover:bg-[#F5A623]/10 transition-all duration-300">
                    <Mail className="w-6 h-6 text-teal-400 group-hover:text-[#F5A623] transition-colors" />
                  </div>
                  <div>
                ...
                <a href="tel:+3216196984" className="flex items-center gap-6 group selectable">
                  <div className="w-14 h-14 rounded-2xl bg-[#011a24] border border-teal-800 flex items-center justify-center group-hover:border-[#F5A623]/50 group-hover:bg-[#F5A623]/10 transition-all duration-300">
                    <Phone className="w-6 h-6 text-teal-400 group-hover:text-[#F5A623] transition-colors" />
                  </div>
                  <div>
                ...
                <div className="flex items-center gap-6 group selectable">
                  <div className="w-14 h-14 rounded-2xl bg-[#011a24] border border-teal-800 flex items-center justify-center group-hover:border-[#F5A623]/50 group-hover:bg-[#F5A623]/10 transition-all duration-300">
                    <MapPin className="w-6 h-6 text-teal-400 group-hover:text-[#F5A623] transition-colors" />
                  </div>
                  <div>
              ...
              <div className="bg-gradient-to-br from-[#023A4E]/60 to-[#011a24]/90 backdrop-blur-2xl p-10 md:p-14 rounded-[3rem] border border-teal-700/30 shadow-[0_20px_60px_rgba(0,0,0,0.5)] relative overflow-hidden">
                ...
                <form onSubmit={handleSubmit} className="space-y-6 relative z-10">
                  <div className="relative bg-[#011a24]/40 border border-teal-800/50 rounded-2xl focus-within:border-[#F5A623]/60 focus-within:bg-[#012330]/60 transition-all duration-300 shadow-inner">
                    ...
                    />
                    <label className="absolute left-5 top-4 text-teal-500/90 text-sm pointer-events-none transition-all peer-focus:-translate-y-2 peer-focus:text-[#F5A623] peer-focus:text-[10px] peer-focus:font-bold peer-focus:tracking-widest peer-[:not(:placeholder-shown)]:-translate-y-2 peer-[:not(:placeholder-shown)]:text-[#F5A623] peer-[:not(:placeholder-shown)]:text-[10px] peer-[:not(:placeholder-shown)]:font-bold peer-[:not(:placeholder-shown)]:tracking-widest">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
          <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-10">
            <div className="flex items-center gap-4 interactive-card cursor-pointer" onClick={() => scrollToSection('home')}>
              <NutriLogo className="w-9 h-9 text-[#F5A623]" cutoutColor="#001720" />
              <div>
            ...
            <div className="flex flex-wrap items-center gap-x-6 gap-y-2 text-[11px] font-bold uppercase tracking-widest">
              <a href="#waarom" onClick={(e) => { e.preventDefault(); scrollToSection('waarom'); }} className="text-teal-300/70 hover:text-[#F5A623] transition-colors">{t.nav.why}</a>
              <a href="#" onClick={(e) => e.preventDefault()} className="text-teal-300/70 hover:text-[#F5A623] transition-colors">{t.footer.privacy}</a>
              <a href="#" onClick={(e) => e.preventDefault()} className="text-teal-300/70 hover:text-[#F5A623] transition-colors">{t.footer.cookies}</a>
              <a href="#" onClick={(e) => e.preventDefault()} className="text-teal-300/70 hover:text-[#F5A623] transition-colors">{t.footer.legal}</a>
            </div>
          </div>
      ...
      <style dangerouslySetInnerHTML={{ __html: `
        ...
        * { cursor: none; }
        ...
        ::selection { background: rgba(245,166,35,0.35); color: white; }
